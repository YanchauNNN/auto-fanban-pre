...
import { getCurrentNodeLabel, getWorkflowStatusLabel } from "../../app/taskGroupPresentation";
import type {
...
const WORKLOAD_SCOPE_LABELS: Record<WorkloadScopeKey, string> = {
  me: "个人",
  office: "科室",
  institute: "全所",
  admin: "管理员",
...
function getAvailableScopes(role: string): WorkloadScopeKey[] {
  if (role === "管理员") {
    return ["me", "office", "institute", "admin"];
  }
  if (role === "所领导") {
    return ["me", "office", "institute"];
  }
  if (role === "室主任") {
    return ["me", "office"];
  }
...
export function WorkloadPage() {
  ...
  const [approvalFactor, setApprovalFactor] = useState("1.00");
  ...
  async function handleApprovalSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!/^\d+(\.\d{1,2})?$/.test(trimmed)) {
      setApprovalError("审批系数请保留两位以内小数。");
      return;
    }

    const factor = Number.parseFloat(trimmed);
    if (Number.isNaN(factor) || factor < 0.8 || factor > 1.1) {
      setApprovalError("审批系数需在 0.80 到 1.10 之间。");
      return;
    }
                ...
                <article className={styles[`monitorCard${capitalize(tone)}`]} key={item.groupId}>
                  <div className={styles.monitorHeader}>
                    ...
                    <span className={styles.monitorBadge}>
                      {getWorkflowStatusLabel(item.workflowStatus)}
                    </span>
                  </div>
                  <dl className={styles.monitorMeta}>
                    ...
                    <div>
                      <dt>当前节点</dt>
                      <dd>{getCurrentNodeLabel(item.currentNodeKey)}</dd>
                    </div>
                    <div>
                      <dt>有效工作量</dt>
      ...
      <section className={styles.section}>
        ...
        >
          ...
          <label className={styles.filterField}>
            ...
            >
              {WORKLOAD_STATUS_OPTIONS.map((option) => (
        ...
        >
          <div className={styles.modalCard}>
            ...
            <form className={styles.form} onSubmit={handleApprovalSubmit}>
              ...
              />
              <p className={styles.helpText}>允许范围 0.80 到 1.10，最多两位小数。</p>
            ...
            <form className={styles.form} onSubmit={handleRepairSubmit}>
              {repairMode === "replace" ? (
                ...
                <>
                  ...
                  <p className={styles.helpText}>新建修复账号时，默认密码固定为 `password`。</p>
                </>
              )}
