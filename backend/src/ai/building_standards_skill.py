            ...
            **gate,
            "policy": {
                ...
                "evidence_gate": (
                    ...
                    "相关正文，且记录质量、风险、规范状态和授权检查通过，才可给出最终设计建议。"
                    "quality_status描述当前记录的文字或单元格质量，不等于允许设计建议。"
                    "record_review仅证明本条款或表格已经过原页转录复核，不能扩展到相邻记录或整页；"
                    "它可以解释为何当前记录已核验而page_quality仍保留旧OCR风险。"
                    "没有record_review时仍须检查覆盖全部页的质量。"
