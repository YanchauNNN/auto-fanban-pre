...
> 范围：**封面（Word+PDF）/目录（Excel+PDF）/设计文件（仅Excel）/IED计划（仅Excel，单独输出）**  
...
- `design_xlsx`（设计文件 Excel，不导出 PDF）
- `ied_xlsx`（IED 计划 Excel，单独输出，不包含在 package.zip 中，也不导出 PDF）
- `manifest.json`（生成清单：记录模板版本、输入参数、推导字段、生成日志摘要、产物路径）

### 1.3 任务提交契约（补充）

当前主程序除了“文档生成字段”本身，还存在一层稳定的“任务提交协议”。这层协议现在已经进入
`documents/参数规范.yaml -> submission_contracts`，推荐统一按下面理解：

- `POST /api/jobs/batch`
  - `multipart/form-data`
  - 文件字段：`files[]`
  - 参数字段：`params_json`
  - 可选表单字段：`run_audit_check`
  - `params_json` = `doc_generation.params` 中所有 `source=frontend` 字段
    + `font_preflight_contract`
    + `plot_style_key`
- `POST /api/jobs/audit-replace`
  - `multipart/form-data`
  - 文件字段：`files[]`
  - 表单字段：`mode`、`params_json`
  - `mode=check` 时只需要 `project_no`
  - `mode=replace` 时需要：
    - `source_project_no`
    - `target_project_no`
    - `run_deliverable`
    - `deliverable_params`（仅 `run_deliverable=true` 时需要；内容与出图参数同构，并额外允许 `plot_style_key`）
...
| `pdf_engine`（导出PDF的实现方式） | Word/Excel → PDF 引擎 | enum | `office_com`（当前部署默认） / `libreoffice`（历史兼容项，默认禁用） |
| `pdf_margin`（PDF页边距设置，适用于DWG/DXF打印） | 上下左右页边距（mm） | dict | 默认 `{top:20, bottom:10, left:20, right:10}`，定义为“PDF 页面边缘 → 图框最大外框线”的距离 |

> 约定：当 `options.enabled == true` 时，**封面/目录/设计文件/IED 全部生成并输出**；其中封面/目录输出 PDF，设计文件与 IED 仅输出 Excel。
...
| `file_category`（文件类别） | 文件类别 | str | 是 | **前端选择**；选项来自 `设计文件模板.xlsx` 的 Sheet5“文件类别”A列；写入设计文件 U列 |
...
| `is_upgrade`（是否升版） | 升版开关 | bool语义字符串 | 否 | 前端输入：`"true"` / `"false"`；默认 `false`；只控制目录升版标记，不参与任何版本号推导 |
| `upgrade_sheet_codes`（升版图纸编号） | 升版图纸编号 | str | 否 | 前端输入：对应 `internal_code` 最后三位；支持 `001~099`、`001、003、005~009`、`1,3,5-9` 等组合；留空表示仅标记目录文件本身为升版 |
| `catalog_revision`（目录版次） | 目录版次 | str | 是 | **派生**：与封面版次保持一致，并取图纸 `001~XXX` 的最高版次；无图纸版次时回退 `cover_revision`（默认A） |
...
- **几何候选（layer_priority）**：先扫描 `global_layers` 的闭合轴对齐多段线/开放4点轴对齐多段线；若仍有未覆盖锚点，再仅在 `local_only_layers` 的局部窗口内执行 LINE/开放多段线重建。
- **LINE重建上限（line_rebuild_limits）**：`max_segments=500000` 表示参与LINE重建的线段数量上限；`max_coord_pairs=1000000` 表示坐标组合上限，用于限制重建矩形的组合规模，防止计算爆炸。
- **纸张拟合**：从几何候选得到 `paper_variant_id/sx/sy/roi_profile_id`。
- **锚点ROI验证**：用 `anchor_roi_rb_offset_1to1` 还原ROI，选择 `fit_error` 更优的候选。
- **SMALL5(A4)簇扩展**：锚点命中的 A4 作为种子扩展整个簇。
- **字段提取/比例校验**：按 ROI profile 提取字段；`scale_check` 仅写 flags 不中断。
...
| `frames[].roi_profile_id`（ROI配置档） | `BASE10/SMALL5` | YAML `roi_profiles` | 决定各字段 ROI 位置 |
| `frames[].scale_mismatch`（比例不一致布尔值） | 几何缩放 vs 图签比例是否不一致 | DXF流水线 `scale_check` | `true` 时需要报警，并写入 `frames[].flags[]`（追加 `比例不一致`） |
...
| `catalog_revision`（目录版次） | 版次 | `H5` | 取 `catalog_revision`：与封面版次保持一致，并取图纸 `001~XXX` 的最高版次；无图纸版次时回退 `cover_revision`（默认A） |
...
| I | 附注 | **升版标记**：`is_upgrade=false` 时全部留空；`is_upgrade=true` 时封面行与目录行固定写项目对应标记（非1818：`升版`；1818：`升版 upgrade`）；若 `upgrade_sheet_codes` 解析后命中图纸 `internal_code` 最后三位，则对应图纸行写相同标记 |
