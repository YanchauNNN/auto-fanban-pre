...
> `DWG→DXF → 识别图框 → 图签字段提取 →（A4多页成组）→ 拆分/裁切 → 重命名 → 输出PDF+DWG → 生成封面/目录（含PDF）+设计文件（仅Excel）→ 打包ZIP + 单独输出IED计划.xlsx`  
> 并提供“词库检查/替换（纠错/翻版）”能力。
>
> **硬约束**：
> - **DXF 仅作为中间文件**，不对用户输出。
> - 首版可跳过字段/XREF（外部参照），不追求完全保真，但需支持 **多图幅、多比例混排**。
> - 文档中仅封面/目录**必须导出 PDF**；设计文件与 IED 仅 Excel 单独输出，不导出 PDF。
>
...
- `titleblock_extract`：几何候选（layer_priority内，poly优先、line末级）→纸张拟合→锚点ROI验证→A4簇扩展 + rb_offset ROI + 字段解析规则
...
**多DWG处理口径（已确认）**
- 每个 DWG **独立执行流水线A**（从转换到文档生成），互不干扰
- 不跨 DWG 合并图框/A4成组/目录排序/派生字段
- 产物按 DWG 文件名分目录组织（打包时保持分隔）
- IED 按 DWG 独立输出（不合并为单一文件）
...
### 4.3 前端必填高亮提醒（已确认）
- 前端根据 `required/required_when` 对字段做**高亮提醒**
- 条件变化时实时刷新高亮状态（例如切换 `project_no/ied_status`）
- 后端仍执行严格校验，前端高亮仅用于提示
...
3. `DETECT_FRAMES_GEOM`：layer_priority内几何候选 + 纸张拟合（几何优先，不依赖锚点字高）
   - 每个 layer 内：闭合且轴对齐的 `LWPOLYLINE/POLYLINE` 优先；开放但 **4点轴对齐** 可直接作为候选
   - 仅当该 layer 没有 polyline 候选时，才执行 LINE 重建（含开放多段线拆线段）
   - LINE 重建受 `line_rebuild_limits` 约束，超限记录日志并跳过该 layer
4. `VERIFY_BY_ANCHOR`：锚点ROI验证并选择候选；无全局兜底；SMALL5 以锚点命中A4为种子扩展簇
5. `EXTRACT_TITLEBLOCK_FIELDS`：按 ROI profile 提取字段并解析（含比例一致性检查）
6. `A4_MULTIPAGE_GROUPING`：对 A4 多页说明图（001）做簇聚类与 Master/Slave 识别
7. `SPLIT_AND_RENAME`：按图框裁切/拆分成子图（保持坐标不归零），重命名
8. `EXPORT_PDF_AND_DWG`：输出子图 PDF（按图幅/页边距）与 DWG（DXF→DWG，必须）
9. `GENERATE_DOCS`：封面/目录生成并导出 PDF；设计文件与 IED 生成 Excel 单独输出（不导出 PDF）
10. `PACKAGE_ZIP`：打包交付包 + 输出 manifest
...
- `enabled`: true 时 **封面/目录生成并导出 PDF；设计文件与 IED 仅 Excel 单独输出**
- `export_pdf`: 必须 true
...
### 7.1 处理流水线（7 步）
`dwg_to_dxf → geom_candidates → scale_fit → anchor_verify → a4_cluster_expand → field_extract → scale_check`
...
- ROI profiles：
  - `BASE10`（大图幅）
  - `SMALL5`（A3/A4 小图幅）
- 比例来自纸张拟合 `sx/sy`；锚点字高推导仅用于 RB 兼容模式
  ...
  backend/
    app/
      ...
      services/
        ...
        frame_detector.py         # 几何候选 + 纸张拟合 + 锚点ROI验证 + A4簇扩展
...
6. A4 多页成组（001 高频场景）
7. 文档生成（封面/目录导出 PDF，设计文件/IED 仅 Excel）（Office COM 优先）
