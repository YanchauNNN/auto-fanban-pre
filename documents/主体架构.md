...
- **`documents/参数规范.yaml`**：唯一权威的结构化规范，包含：
  ...
  - `titleblock_extract`：几何候选→纸张拟合→锚点ROI验证→A4簇扩展 + rb_offset ROI + 字段解析规则
...
3. `DETECT_FRAMES_GEOM`：layer_priority内几何候选（闭合多段线优先、开放4点、LINE重建）+ 纸张拟合
4. `VERIFY_BY_ANCHOR`：锚点ROI验证并选择候选；SMALL5 以锚点命中A4为种子扩展簇
